defmodule PlaygroundWeb.CoreComponents do
  ...
  slot :configuration, required: false
  attr :rest, :global
  slot :example, required: true do
    attr :name, :string, required: true
  end
  ...
  def showcase(assigns) do
    ~H"""
      ...
      <div class="showcase-header mb-4">
        <h2 class="text-xl font-bold"><%= render_slot(@title) %></h2>
        <p :if={@details != []} class="text-gray-700"><%= render_slot(@details) %></p>
        <div :if={@configuration != []} class="showcase-configuration mt-2 p-2 bg-gray-100 rounded">
          <%= render_slot(@configuration) %>
        </div>
      </div>
      <div class="showcase-examples flex-1 flex flex-col">
